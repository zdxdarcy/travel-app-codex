...
function queueSelectedExtras() {
  ...
  const load = guideClient.loadAttractionExtras(selected.placeId).then(({ media, reviews }) => {
    ...
    const card = document.querySelector(`[data-detail-card="${CSS.escape(selected.id)}"]`);
    const content = card?.querySelector("[data-detail-extra]");
    if (content) content.innerHTML = detailPlaceDetails(selected.details);
